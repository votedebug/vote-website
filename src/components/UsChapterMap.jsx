...
// Three tiers, darkest where we are most established. Literal values rather
// than tokens because SVG `fill` cannot take an opacity-modified var.
const TIER_FILL = {
  directors: 'var(--color-navy)',
  chapters: '#7d99d4',
  none: '#e2e6ef',
}
 ...
 *
 * The map runs the full width of the page and the state index sits underneath,
 * so the country is the thing you see first rather than a column beside a list.
 *
        ...
        {paths.map((s) => {
          ...
          return (
            <path
              ...
              aria-label={clickable ? `${s.name}: ${chaptersByState[s.abbr].length} chapters — open` : undefined}
        ...
        <ul className="mt-6 flex flex-wrap items-start justify-center gap-x-14 gap-y-8">
          {active.map((s) => (
            <li key={s.abbr}>
              ...
              >
                <span className="font-serif text-3xl font-semibold leading-none text-navy/25 transition-colors group-hover:text-flag-red">
                  {s.abbr}
                </span>
                <span className="mt-2 font-serif text-lg font-semibold leading-tight text-navy">{s.name}</span>
                ...
                <span className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-flag-red opacity-0 transition-opacity group-hover:opacity-100">
